    ...
    <div className="p-8">
      ...
      {viewingEmail && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-gray-900 p-6 rounded-lg w-full max-w-4xl max-h-[90vh] flex flex-col">
            <h2 className="text-lg font-bold mb-1">{viewingEmail.subject}</h2>
            <p className="text-sm text-gray-400 mb-4">From: {viewingEmail.from_address}</p>
            <div className="overflow-y-auto flex-1 rounded-lg mb-4" style={{ height: "550px" }}>
              {bodyLoading ? (
                <p className="text-blue-400">Loading email...</p>
              ) : emailBody.isHtml ? (
                <iframe srcDoc={emailBody.content} className="w-full h-full rounded-lg border-0" sandbox="allow-same-origin" title="Email content" />
              ) : (
                <pre className="text-sm text-gray-800 whitespace-pre-wrap bg-white p-4 rounded-lg h-full">{emailBody.content}</pre>
              )}
            </div>
            <div className="flex gap-3 justify-end">
              <button onClick={() => { handleReply(viewingEmail); setViewingEmail(null); }} className="px-4 py-2 bg-blue-600 rounded-lg hover:bg-blue-500 text-white">Reply</button>
              <button onClick={() => setViewingEmail(null)} className="px-4 py-2 bg-gray-700 rounded-lg hover:bg-gray-600 text-white">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {replyingTo && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-gray-900 p-6 rounded-lg w-full max-w-lg">
            <h2 className="text-lg font-bold mb-1">Reply to {replyingTo.from_address}</h2>
            <p className="text-sm text-gray-400 mb-4">Re: {replyingTo.subject}</p>
            {replyLoading ? (
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setReplyingTo(null)} className="px-4 py-2 bg-gray-700 rounded-lg hover:bg-gray-600">Cancel</button>
              <button onClick={handleSend} disabled={sending || replyLoading} className="px-4 py-2 bg-blue-600 rounded-lg hover:bg-blue-500 disabled:opacity-50">
          ...
          <div key={email.gmail_id} className="border border-gray-700 p-4 mb-2 rounded">
            <div className="flex justify-between items-start">
              ...
              <div className="flex gap-3">
                <button onClick={() => handleView(email)} className="text-xs text-green-400 hover:text-green-300">View</button>
                <button onClick={() => handleReply(email)} className="text-xs text-blue-400 hover:text-blue-300">Reply</button>
      ...
      {nextPageToken && (
        <div className="flex justify-center mt-6">
          <button onClick={handleLoadMore} disabled={loadingMore} className="bg-gray-700 text-white px-6 py-3 rounded-lg hover:bg-gray-600 disabled:opacity-50">
